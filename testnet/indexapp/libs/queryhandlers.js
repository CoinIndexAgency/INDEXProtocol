...
exports.Handlers = {
	doQuery: function(path, data, appState, indexProtocol){
		...
		switch( path ) {
			...
			case 'tbl.commit' : {
				return this._tblCommitAtBlock(data, appState, indexProtocol);
				break;
			}
			default: {
				return {code: 1};
	...
	_tblCommitAtBlock: function(data, appState, indexProtocol){
		let _height = parseInt( data.toString('utf8') );
				
		if (!_height || _height > appState.blockHeight)
			return { code: 1 };
		...
		console.log('REQUEST: tbl.commit.' + _height); 
				
		return new Promise(function(resolve, reject){
					
			indexProtocol.ssdb.hgetall('tbl.commit.'+_height, function(err, val){
				//console.dir( err, {depth:2} );	
				console.dir( val, {depth:8} );	
				
				if (!err && val){
					//console.log('Query fetch: avg tx from block #' + _height);
					
					_.each(val, function(json, key){
						val[key] = JSON.parse( json );
					});
